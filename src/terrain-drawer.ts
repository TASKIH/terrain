...
export class TerrainDrawer {
    ...
    // 等高線の作成
    static contour(h: TerrainHeights, level: number) {
        ...
        for (var i = 0; i < h.mesh!.edges.length; i++) {
            ...
            if (TerrainCalcUtil.isNearEdge(h.mesh!, edge.index1) || TerrainCalcUtil.isNearEdge(h.mesh!, edge.index2))
                continue;
    ...
    static drawMap(svg: any, render: MapRender) {
        render.rivers = TerrainFeatureGenerator.getRivers(render.h, 0.01);
    ...
    static visualizeDownhill(h: TerrainHeights) {
        var links = TerrainFeatureGenerator.getRivers(h, 0.01);
    ...
    static visualizeSlopes(svg: any, render: MapRender) {
        ...
        for (var i = 0; i < h.length; i++) {
            if (h[i] <= 0 || TerrainCalcUtil.isNearEdge(h.mesh, i)) continue;
            var nbs = TerrainCalcUtil.getNeighbourIds(h.mesh, i);
            nbs.push(i);
            var s = 0;
            var s2 = 0;
            for (var j = 0; j < nbs.length; j++) {
                var slopes = TerrainGenerator.trislope(h, nbs[j]);
                s += slopes[0] / 10;
                s2 += slopes[1];
            }
            s /= nbs.length;
