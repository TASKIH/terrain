export function shuffled(list: any) {
    var newlist = [];
    for (var i = 0; i < list.length; i++) {
        newlist.push(list[i]);
    }
    for (var i = list.length - 1; i > 0; i--) {
        var tmp: any = newlist[i];
        var j = randrange(i);
        newlist[i] = newlist[j];
        newlist[j] = tmp;
    }
...
export function choose(list: any, exponent?: any) {
    exponent = exponent || 1;
    return list[Math.floor(Math.pow(Math.random(), exponent) * list.length)];
}

export function randrange(lo: any, hi?: any) {
    if (hi == undefined) {
        hi = lo;
        lo = 0;
    }
...
export function join(list: any, sep: any) {
...
export function spell(lang: any, syll: any) {
...
export function makeSyllable(lang: any) {
    while (true) {
        var syll = "";
...
export function getMorpheme(lang: any, key: any) {
    if (lang.nomorph) {
...
export function makeWord(lang: any, key: any) {
...
export function getWord(lang: any, key: any) {
    key = key || '';
...
export function makeName(lang: any, key: any) {
    key = key || '';
...
export function makeRandomLanguage() {
    var lang: any = makeBasicLanguage();
    lang.noortho = false;
    lang.nomorph = false;
